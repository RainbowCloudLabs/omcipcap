...
PCAP_OUTPUT = Path("examples/issues/case_03_second_gem_vlan_rule_missing.pcap")

tid = 1
pkts = []


def build_std_me_data(
    me_class, attr_mask, attr_values, is_mib_upload=True, is_create=False
):
    """
...
def gen_mib_upload_pkts():
    ...
    card_data = build_std_me_data(
        OMCIClass.CARDHOLDER, 0xF000, [0xEE, 0xEE, 1, "SFU10G"]
    )
    ...
    circuit_data = build_std_me_data(
        OMCIClass.CIRCUIT_PACK,
        0xF000,
        [0xEE, 1, b"\x50\x43\x41\x50\x00\x00\x00\x01", "V1.0"],
    )
    ...
    circuit_data = build_std_me_data(
        OMCIClass.CIRCUIT_PACK,
        0xF000,
        [0x31, 1, b"\x50\x43\x41\x50\x00\x00\x00\x01", "V1.0"],
    )
    ...
    ts_data = build_std_me_data(
        OMCIClass.TRAFFIC_SCHEDULER_G, 0xF000, [0x8000, 0, 2, 0]
    )
    mib_data.append((OMCIClass.TRAFFIC_SCHEDULER_G, 0x8000, 0xF000, ts_data))
    ts_data = build_std_me_data(
        OMCIClass.TRAFFIC_SCHEDULER_G, 0xF000, [0x8001, 0, 2, 0]
    )
    mib_data.append((OMCIClass.TRAFFIC_SCHEDULER_G, 0x8001, 0xF000, ts_data))
    # PPTP 10G
    pptp_data = build_std_me_data(
        OMCIClass.PPTP_ETHERNET_UNI, 0xFF00, [0x31, 0x31, 0, 0, 0, 0, 0, 0x05EE]
    )
...
def gen_provisioning_pkts(start_tid=0x100):
    ...
    pkts.append(
        create_omci(
            tid,
            msg_req(OmciAction.CREATE),
            OMCIClass.MAC_BRIDGE_SERVICE_PROFILE,
            0x1,
            content=build_std_me_data(
                OMCIClass.MAC_BRIDGE_SERVICE_PROFILE,
                0,
    ...
    pkts.append(
        create_omci(
            tid,
            msg_req(OmciAction.CREATE),
            OMCIClass.GAL_ETHERNET_PROFILE,
            10,
            content=build_std_me_data(
                OMCIClass.MAC_BRIDGE_PORT_CONFIGURATION_DATA,
                0,
                [0x07D0],
                is_mib_upload=False,
                is_create=True,
    ...
    pkts.append(
        create_omci(
            ...
            0x8000,
            content=build_std_me_data(
                OMCIClass.T_CONT, 0x8000, [1000], is_mib_upload=False
            ),
        )
    )
    pkts.append(
    ...
    pkts.append(
        create_omci(
            tid,
            msg_req(OmciAction.CREATE),
            OMCIClass.DOT1P_MAPPER_SERVICE_PROFILE,
            1,
            content=build_std_me_data(
                OMCIClass.DOT1P_MAPPER_SERVICE_PROFILE,
                0,
    ...
    pkts.append(
        create_omci(
            tid,
            msg_req(OmciAction.CREATE),
            OMCIClass.GEM_INTERWORKING_TERMINATION_POINT,
            0x1,
            content=build_std_me_data(
                OMCIClass.GEM_INTERWORKING_TERMINATION_POINT,
                0,
    ...
    pkts.append(
        create_omci(
            tid,
            msg_req(OmciAction.CREATE),
            OMCIClass.VLAN_TAGGING_FILTER_DATA,
            0x2,
            content=build_std_me_data(
                OMCIClass.VLAN_TAGGING_FILTER_DATA,
                0,
...
def main():
    ...
    generate_pcap_from_pkts(str(PCAP_OUTPUT), mib_uploads_pkts + mib_actions_pkts)
